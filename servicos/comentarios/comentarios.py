...
def get_info():
    return jsonify(
        descricao = "gerenciamento de comentarios do D2 Best Builds",
        versao = "1.0"
    )
...
def get_comentarios(feed, ultimo_comentario, tamanho_pagina):
    ...
    cursor.execute("SELECT id as comentario_id, feed as buildDestiny_id, comentario, nome, conta, DATE_FORMAT(data, '%Y-%m-%d %H:%i') as data " +
                   "FROM comentarios " +
