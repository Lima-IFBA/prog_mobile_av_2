...
class DetalhesState extends State<Detalhes> {
  ...
  bool _temBuildDestiny = false;
  dynamic _buildDestiny;
  ...
  void _carregarBuildDestiny() {
    _servicoBuildsDestiny
        .findBuildDestiny(estadoApp.idBuildDestiny)
        .then((buildDestiny) {
      _buildDestiny = buildDestiny;

      if (estadoApp.usuario != null) {
        _servicoCurtidas
            .curtiu(estadoApp.usuario!, estadoApp.idBuildDestiny)
            .then((curtiu) {
          setState(() {
  ...
  void _carregarComentarios() {
    ...
    _servicoComentarios
        .getComentarios(
            estadoApp.idBuildDestiny, _ultimoComentario, TAMANHO_DA_PAGINA)
        .then((comentarios) {
      setState(() {
  ...
  void _adicionarComentario() {
    _servicoComentarios
        .adicionar(estadoApp.idBuildDestiny, estadoApp.usuario!,
  ...
  Widget _exibirMensagemComentariosInexistentes() {
    return const Center(
        child: Padding(
            ...
            child: Text('não existem comentários sobre esta build',
      ...
      _temComentarios
          ? Expanded(
              ...
              listEmptyWidget: Container(
                  ...
                  child: const Text("Não existem builds para exibir")),
            ))
  ...
  List<String> _imagensDoSlide() {
    ...
    imagens.add(_buildDestiny["imagem1"]);
    if ((_buildDestiny["imagem2"] as String).isNotEmpty) {
      imagens.add(_buildDestiny["imagem2"]);
    }
    if ((_buildDestiny["imagem3"] as String).isNotEmpty) {
      imagens.add(_buildDestiny["imagem3"]);
    }
  ...
  Widget _exibirBuildDestiny() {
            ...
            Align(
                ...
                child: Column(children: [
                  ...
                  IconButton(
                      onPressed: () {
                        final texto =
                            '${_buildDestiny["nome_buildDestiny"]} disponível no D2 Best Builds.\n\n\nBaixe o D2 Best Builds na PlayStore!';
            ...
            children: [
              _temBuildDestiny
                  ? Padding(
                      ...
                      child: Text(_buildDestiny["descricao"],
                          style: const TextStyle(fontSize: 12)))
                  : const SizedBox.shrink(),
              _temBuildDestiny
                  ? Padding(
                      ...
                      child: Row(children: [
                        Text(
                          "Popularidade: ${_buildDestiny["popularidade"]}",
                        ...
                        Padding(
                            ...
                            child: Row(
                                ...
                                children: [
                                  ...
                                  Text(
                                    _buildDestiny["curtidas"].toString(),
                                    style: const TextStyle(
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Row(children: [
          Row(children: [
            Image.network(caminhoArquivo(_buildDestiny["avatar"]), width: 38),
            Padding(
                padding: const EdgeInsets.only(left: 10.0, bottom: 5.0),
                child: Text(
                  _buildDestiny["nome_buildDestiny"],
  ...
  Widget _exibirMensagemBuildDestinyInexistente() {
    return Scaffold(
      ...
      const Material(
          ...
          child: Text('Essa build não existe ou foi removido :-(',
              style: TextStyle(color: Colors.black, fontSize: 14))),
    ])));
  }
  ...
  Widget build(BuildContext context) {
    return KeyboardVisibility(
        ...
        child: _temBuildDestiny
            ? _exibirBuildDestiny()
            : _exibirMensagemBuildDestinyInexistente());
  }
}
